...
<div>
    ...
    <div class="flex flex-wrap gap-2 mb-6">
        ...
        @foreach($allTags as $tag)
            <button
                ...
                {{ $tag === 'All'
                    ? TechnologyColors::light('All')
                    : TechnologyColors::light(
                        config("skill-categories.technologies.$tag", 'All')
                    )
                }}"
            >
                {{ $tag }}
            ...
            >
                ...
                <div class="aspect-video overflow-hidden relative bg-stone-100">
                    ...
                    @if($project->thumbnail)
                        ...
                        >
                    @else
                        <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-stone-50 to-stone-100">
                            <span class="font-serif text-3xl text-stone-300">
                ...
                <div class="p-4">
                    ...
                    @php
                        $primaryTech = $project->tech_tags[0] ?? 'Laravel';
                    @endphp

                    @if($project->category)
                        <p class="text-[10px] uppercase tracking-wide mb-1
                            {{ str_contains(TechnologyColors::badge($primaryTech), 'violet') ? 'text-violet-600' : '' }}
                            {{ str_contains(TechnologyColors::badge($primaryTech), 'emerald') ? 'text-emerald-600' : '' }}
                            {{ str_contains(TechnologyColors::badge($primaryTech), 'amber') ? 'text-amber-600' : '' }}
                            {{ str_contains(TechnologyColors::badge($primaryTech), 'slate') ? 'text-slate-600' : '' }}
                        ">
                            {{ $project->category }}
                        </p>
                    @endif
    ...
    {{-- ── PROJECT MODAL ─────────────────────────────────────────────── --}}
    @if($openProject)
    {{-- Only add this overflowhidden when model is open --}}
    @php
        echo '<script>document.body.classList.add("overflow-hidden")</script>';
    @endphp
    <div
        x-data
